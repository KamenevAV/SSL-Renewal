...
load_config() {
  ...
  source "$cfg"
  export ROLE APP_DIR ETC_DIR PRIMARY_DOMAIN EXTRA_DOMAINS_CSV TARGET_DIR CERT_DIR LOG_DIR TELEGRAM_ENABLED TELEGRAM_BOT_TOKEN TELEGRAM_CHAT_ID NODES_FILE DNS_PROPAGATION_SECONDS CLOUDFLARE_CREDENTIALS REGION_WILDCARDS_CSV SSH_CONNECT_TIMEOUT SSH_COMMAND_TIMEOUT DEPLOY_RETRIES DEPLOY_RETRY_DELAY MIN_CERT_VALIDITY_SECONDS DEPLOY_LOCK_FILE
}
